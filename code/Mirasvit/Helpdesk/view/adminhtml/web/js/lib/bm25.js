...
BM25.Tokenize = function(text) {
    ...
    for (var i = 0, len = text.length; i < len; i++) {
        ...
        if (stopStems.indexOf(word) === -1) {
            if (word.length > 3) {
                for (var j = 3, strLen = word.length; j <= strLen; j++) {
                    out.push(word.substring(0, j));
                }
            } else {
                out.push(word);
            }
...
BM25.prototype.addDocument = function(doc) {
    ...
    if (typeof doc.name === 'undefined') { throw new Error(1001, 'Name is a required property of documents.'); };
    ...
    var docObj = {id: doc.id, tokens: {}, body: doc.body, terms: {}};
    
    if (typeof doc.name !== 'undefined') {
        docObj.name = doc.name;
    
        this.calcTerms(docObj, docObj.name, 100);
    }
    
    this.calcTerms(docObj, docObj.body, 0);
    
    // Add docObj to docs db
    this.documents[docObj.id] = docObj;
};

BM25.prototype.calcTerms = function(docObj, text, coef) {
    
    // Raw tokenized list of words
    var tokens = BM25.Tokenize(text);
    
    // Count number of terms
    docObj.tokens    = tokens;
    ...
    for (var i = 0, len = tokens.length; i < len; i++) {
        var term = tokens[i];
        if (typeof docObj.terms[term] == 'undefined') {
            docObj.terms[term] = {
                count: 0,
                freq: 0,
                count: coef
        ...
        docObj.terms[term].count++;
    }
    
    // Then re-loop to calculate term frequency.
    // We'll also update inverse document frequencies here.
    var keys = Object.keys(docObj.terms);
    for (var i = 0, len = keys.length; i < len; i++) {
        var term = keys[i];
        // Term Frequency for this document.
        docObj.terms[term].freq = docObj.terms[term].count / docObj.termCount;
...
BM25.prototype.search = function(query) {
    ...
    var queryTerms = BM25.SplitText(query);
    var results    = [];
