...
class SubjectColumn extends Column
{
    ...
    protected function addQuickViewInfo(array &$item)
    {
        ...
        $firstMessage = $collection->addFieldToFilter(TicketInterface::KEY_ID, $item[TicketInterface::KEY_ID])
            ->setOrder('main_table.created_at', 'asc')->setPageSize(1)->getFirstItem();

        $lastMessage = $collection->addFieldToFilter(TicketInterface::KEY_ID, $item[TicketInterface::KEY_ID])
            ->setOrder('main_table.created_at', 'desc')->setPageSize(1)->getFirstItem();
