...
class Collection extends \Magento\Catalog\Model\ResourceModel\Product\Collection
{
    ...
    {
        // Set default order by entity_id for consistent pagination
        $this->setOrder('entity_id');

        // save existing order to reapply it later
        $orderParts = $this->getSelect()->getPart(Select::ORDER);

        // Reset the order to prevent conflicts before adding custom sorting
        $this->getSelect()->reset(Select::ORDER);

        // try to sort by stock status, if not possible it will be skipped and original sorting applied
        try {
            // Only add filter if stock status index table isn't already part of the query
            $fromTables = $this->getSelect()->getPart('from');
            if (!isset($fromTables['stock_status_index'])) {
                $this->stockHelper->addIsInStockFilterToCollection($this);
            }

            // Add stock sorting logic based on whether stock status is available
            $fromTables = $this->getSelect()->getPart('from');
            if (isset($fromTables['stock_status_index'])) {
                // is_saleable not available in legacy stock system so if default table is used then fallback to stock status
                $isMsi = $this->isMsiEnabled() && $fromTables['stock_status_index']['tableName'] !== $this->getResource()->getTable('cataloginventory_stock_status');
                $stockStatusAlias = $isMsi ? 'stock_status_index.is_salable' : 'stock_status_index.stock_status';

                // Sort by stock status (descending order: in-stock first)
                $this->getSelect()->order($stockStatusAlias . ' DESC');
            } else {
                // If stock status table isn't available, sort by in-stock flag (descending)
                $this->getSelect()->order('is_salable DESC');
            }
        } catch (\Exception $e) {
            $this->logger->error('Olegnax Layered Navigation: Error when trying to sort by stock status: ' . $e->getMessage());
        }

        // Re-apply existing order parts after adding in-stock filter
        foreach ($orderParts as $orderPart) {
            if (!is_array($orderPart) && strpos($orderPart, 'stock_status_index') === false) {
                $this->getSelect()->order($orderPart);
            }
        }
    ...
    /**
     * Detect if MSI is enabled by checking Magento_Inventory modules
     */
