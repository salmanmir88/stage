    ...
    public function getAdvancedMatchingData()
{
    ...
    if ($customerSession->isLoggedIn()) {
        ...
        $advancedMatchingData = [
            'em' => $customer->getEmail() ? strtolower($customer->getEmail()) : '',
            'fn' => $customer->getFirstname() ? strtolower($customer->getFirstname()) : '',
            'ln' => $customer->getLastname() ? strtolower($customer->getLastname()) : '',
            'ge' => $customer->getGender() ? ($customer->getGender() == 2 ? 'f' : 'm') : '',
            'db' => $customer->getDob() ? str_replace('-', '', $customer->getDob()) : ''
        ];

        if ($defaultShippingAddress) {
            $advancedMatchingData['country'] = $defaultShippingAddress->getCountryId() ? strtolower($defaultShippingAddress->getCountryId()) : '';
            $advancedMatchingData['ct']      = $defaultShippingAddress->getCity() ? strtolower($defaultShippingAddress->getCity()) : '';
            $advancedMatchingData['st']      = $defaultShippingAddress->getRegionCode() ? strtolower($defaultShippingAddress->getRegionCode()) : '';
            $advancedMatchingData['zp']      = $defaultShippingAddress->getPostcode() ? strtolower($defaultShippingAddress->getPostcode()) : '';
            $advancedMatchingData['ph']      = $defaultShippingAddress->getTelephone() ? preg_replace('~^(\+)|\D~', '\1', $defaultShippingAddress->getTelephone()) : '';
        }
