...
class MetadataProvider extends \Magento\Ui\Model\Export\MetadataProvider
{
    ...
    protected function getActiveColumns($component){
        ...
        $columns = array_filter($config['current']['columns'], fn($config, $key) => $config['visible'] && !in_array($key, ['ids', 'actions', 'facebook_finish', 'facebook_product_weight', 'facebook_shoe_width', 'version', 'facebook_package_quantity', 'album_qyt', 'facebook_optical_zoom', 'dealer', 'facebook_gemstone','facebook_age_group', 'facebook_gender', 'facebook_pattern', 'facebook_decor_style', 'facebook_color', 'facebook_capacity', 'facebook_material', 'facebook_size', 'facebook_style', 'facebook_brand', 'facebook_product_length', 'facebook_product_width', 'facebook_product_height', 'facebook_model', 'facebook_product_depth', 'facebook_ingredients', 'facebook_resolution', 'facebook_age_range', 'facebook_screen_size', 'facebook_maximum_weight', 'facebook_minimum_weight', 'facebook_display_technology', 'facebook_operating_system', 'facebook_is_assembly_required', 'facebook_storage_capacity', 'facebook_number_of_licenses', 'facebook_product_form', 'facebook_compatible_devices', 'facebook_video_game_platform', 'facebook_system_requirements', 'facebook_baby_food_stage', 'facebook_recommended_use', 'facebook_digital_zoom', 'facebook_scent', 'facebook_health_concern', 'facebook_megapixels', 'facebook_thread_count', 'ikon_mini_album_new_kids_conti','bts_bangtan_boys_be_album_card','album_ver','bts_ver','nct_127', 'bts_members','nct_dream','bt21_characters','way_v','member','love_or_take_ver','product_common_code','newest_first','featured','NCT','treasure_versions','exo_member','exo_version','kingdom8102',
        'version_8804775252655','version_8809848758119'  ]), 
            ...
            array_map(
                function($field) use ($columnsType,$document) {
                    if ($field == 'attribute_set_id') {
                        ...
                        $attribute = $document->getResource()->getAttribute($field);

                        if ($attribute && $attribute->usesSource()) {
                            // Safely retrieve the attribute text
                            $columnData = $document->getAttributeText($field);
                            if (is_array($columnData)) {
                                $columnData = implode(',', $columnData);
                            }
                        } else {
                            // Fallback to raw column data if attribute or source is invalid
